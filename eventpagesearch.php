...
<body>
    ...
    <div class="touristGuider_header">
          ...
          <div><h1 class="text-center my-5 py-3 mx-auto">Matching Upcoming Events Found: <span style="color: green; font-size:32px"> <?php echo $_GET["result"] ?> </span>  </h1></div>
         <!-- Tourist Data  -->
        <?php 
            $conn = mysqli_connect("localhost","root","","epaktourism") or die("Connection Failed:" .mysqli_connect_error());
             ...
             $result=$_GET['result'];
               ...
               $sqlrecord="SELECT * FROM tourevents WHERE eventorg LIKE '%{$result}%' OR eventdays LIKE '%{$result}%' OR type LIKE '%{$result}%' OR destinationform LIKE '%{$result}%' OR destinationto LIKE '%{$result}%' OR place1 LIKE '%{$result}%' OR place2 LIKE '%{$result}%' OR place3 LIKE '%{$result}%' OR place4 LIKE '%{$result}%'
               OR place5 LIKE '%{$result}%' OR place6 LIKE '%{$result}%' OR service1 LIKE '%{$result}%' OR service2 LIKE '%{$result}%' OR service3 LIKE '%{$result}%' OR service4 LIKE '%{$result}%' OR service5 LIKE '%{$result}%' OR service6 LIKE '%{$result}%' OR perhead LIKE '%{$result}%'  ORDER BY event_id  DESC LIMIT  {$offset},{$limit}"; 
