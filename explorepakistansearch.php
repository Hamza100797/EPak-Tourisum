...
<body>
    ...
    <div class="menu_header header" id="myHeader">
      <?php include  "particles/mainmenu.php"?>
        ...
        <div><h1 class="text-center my-5 py-3 mx-auto">Matching Blogs Found: <span style="color: green; font-size:32px"> <?php echo $_GET["result"] ?> </span>  </h1></div>
            ...
            $conn = mysqli_connect("localhost","root","","epaktourism") or die("Connection Failed:" .mysqli_connect_error());
             ...
             $result=$_GET['result'];
               ...
               $offset = ($pageNo -1 )* $limit;
                       ...
                       $sqlrecord="SELECT * FROM expolrepakistan WHERE displayname LIKE '%{$result}%' OR posteddate LIKE '%{$result}%' OR destination LIKE '%{$result}%' OR province LIKE '%{$result}%' OR nearestlocation LIKE '%{$result}%' OR aeratag1 LIKE '%{$result}%' OR aeratag2 LIKE '%{$result}%' OR aeratag3 LIKE '%{$result}%' OR aeratag4 LIKE '%{$result}%' ORDER BY post_id  DESC LIMIT  {$offset},{$limit}"; 
