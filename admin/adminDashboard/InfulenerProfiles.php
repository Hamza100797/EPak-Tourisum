...
<div class="wrapper">
  ...
  <?php  include "particles/sidebar.php"?>
    ...
    <section class="content">
       <h1 class="text-center">All Infulencer Stories </h1>
       </section>
       <?php 
            $conn = mysqli_connect("localhost","root","","epaktourism") or die("Connection Failed:" .mysqli_connect_error());
             $limit=6;
              ...
              $sqlrecord="SELECT * FROM infulencer ORDER BY influncer_id  DESC LIMIT  {$offset},{$limit}";                     
         ...
         <main class="container">
            ...
            <article class="postcard red mx-5 " style="margin: 0px 150px;">
                <a class="postcard__img_link" href="#">
                    <img class="postcard__img" src="../InfulencerDashboard/Infulencer_Dashboard/upload/<?php echo$row['image']?>" alt="Image Title" />	
                </a>
                <div class="postcard__text">
                    ...
                    <li><a href="deletestories.php?id=<?php echo$row['influncer_id'] ?>" data-toggle="tooltip" data-placement="bottom" title="Delete Profile Card" data-original-title="Delete Profile Card" data-abc="true" style="font-size: 64px;"><i class="fas fa-trash text-danger mx-2"></i></a></li> 
                     <li><a href="updatestories.php?id=<?php echo$row['influncer_id'] ?>" data-toggle="tooltip" data-placement="bottom" title="Edit Profile Card" data-original-title="Edit Profile Card" data-abc="true" style="font-size: 64px;"><i class="fas fa-user-edit text-success"></i></a></li>
                    </ul>
                </div>
            </article>
        </main>
        ...
        <?php 
                ...
                if(mysqli_num_rows($paginationResult)>0){
                        ...
                        if ($pageNo>1){
                            echo '<li class="page-item"><a class="page-link" href="InfulenerProfiles.php?page='.($pageNo - 1).'">Previous</a></li>';
                        }
                        for($i=1; $i<=$totalpages; $i++){
                          ...
                          echo '<li class='.$active.'> <a href="InfulenerProfiles.php?page='.$i.'"> '.$i.'</a> </li>';
                        }
                        if ($totalpages>$pageNo){
                            echo '<li class="page-item"><a class="page-link" href="InfulenerProfiles.php?page='.($pageNo + 1).'">Next</a></li>';
                        }
                        echo "</ul>";
                }
                ?>
